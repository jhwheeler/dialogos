...
import { TokenExchangeApiInputSchema, TokenExchangeApiOutputSchema, } from "../../types/api/auth/index.js";
...
export async function authRoutes(app) {
    if (process.env.NODE_ENV !== "production") {
        app.post("/auth/dev-token", async (request, reply) => {
            ...
            const token = await signToken({ studentId: validation.data.studentId }, app.container.env.JWT_SECRET);
    ...
    app.post("/auth/token", async (request, reply) => {
        ...
        const studentService = app.container.services.student;
