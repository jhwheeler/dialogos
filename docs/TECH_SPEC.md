...
- Source entity + CRUD + session linking
- Trivium stage selection per session
- Session CRUD + state machine (draft → active → ended → aborted)
- Turn creation + audio upload + presigned URLs
- STT integration (TRANSCRIBE_TURN job)
- LLM integration with Socrates enforcement pipeline (GENERATE_PROMPT job)
- Deterministic Socratic rules enforcement (Section 4.3)
- Source-anchoring rules enforcement (Section 4.4)
- Transcript + summary + rubric artifact generation (RENDER_ARTIFACTS job)
- Per-session metrics computation + storage
- Basic session review screen
- Session screen UI (audio-first, dark, minimal)
